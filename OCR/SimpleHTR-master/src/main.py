...
class ReadTextData:
    ...
    def train(self, model: Model,
        ...
        preprocessor = Preprocessor(self.get_img_size(
        ...
        while True:
            ...
            # validate
            char_error_rate, word_accuracy = self.validate(
                model, loader, line_mode)

            # write summary
            summary_char_error_rates.append(char_error_rate)
            summary_word_accuracies.append(word_accuracy)
            self.write_summary(summary_char_error_rates,
                               summary_word_accuracies)
    ...
    def validate(self, model: Model, loader: DataLoaderIAM, line_mode: bool) -> Tuple[float, float]:
        ...
        preprocessor = Preprocessor(
            self.get_img_size(line_mode), line_mode=line_mode)
        num_char_err = 0
        num_char_total = 0
        num_word_ok = 0
        num_word_total = 0
    ...
    def infer(self, model: Model, fn_img: Path) -> None:
        ...
        preprocessor = Preprocessor(
            self.get_img_size(), dynamic_width=True, padding=16)
    ...
    def getData(self):
        ...
        decoder_type = decoder_mapping[self.decoder]

        # train or validate on IAM dataset
        if self.mode in ['train', 'validate']:
            # load training data, create TF model
            loader = DataLoaderIAM(
                self.data_dir, self.batch_size, fast=self.fast)
            char_list = loader.char_list

            # when in line mode, take care to have a whitespace in the char list
            if self.line_mode and ' ' not in char_list:
                char_list = [' '] + char_list
            ...
            if self.mode == 'train':
                ...
                self.train(model, loader, line_mode=self.line_mode,
                           early_stopping=self.early_stopping)
            elif self.mode == 'validate':
                model = Model(char_list, decoder_type, must_restore=True)
                self.validate(model, loader, self.line_mode)

        # infer text on test image
        elif self.mode == 'infer':
            model = Model(list(open(FilePaths.fn_char_list).read()),
                          decoder_type, must_restore=True, dump=self.dump)
            self.infer(model, self.img_file)
